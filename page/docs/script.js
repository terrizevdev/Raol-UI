      ...
      if (!response.ok) {
        ...
        if (settings.apiSettings?.requireApikey) {
          if (response.status === 401 || response.status === 403) {
            showToast(errorData.message || "Invalid API key", "error", "API Key Error")
            DOM.modal.spinner.classList.add("d-none")
            DOM.modal.container.classList.add("d-none")
            DOM.modal.content.innerHTML = ""
            return
          } else if (response.status === 429) {
            showToast(errorData.message || "Rate limit exceeded", "error", "Rate Limit")
            DOM.modal.spinner.classList.add("d-none")
            DOM.modal.container.classList.add("d-none")
            DOM.modal.content.innerHTML = ""
            return
        ...
        showToast(errorData.message || response.statusText, "error", "API Error")
